...
Latest release: [v0.2.2](https://github.com/jadistanbelly/sts2-multiplayer-save-slots/releases/tag/v0.2.2)

Download `MultiplayerSaveSlots-v0.2.2.zip`, then extract the included `MultiplayerSaveSlots/` folder into your STS2 `mods/` directory.
...
```bash
scripts/release-local.sh --package-only v0.2.2
```

Publish a full release from a clean, synced `main` checkout:

```bash
scripts/release-local.sh v0.2.2
```
...
```bash
scripts/release-local.sh --package-only v0.2.2
scripts/smoke-setup-local.sh install --tag v0.2.2
```
